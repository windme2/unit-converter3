...
import { weightConversionRates, unitLabels } from "../utils/constants";
import Header from "../components/Header";

const WeightConverter = ({ isDark, toggleDarkMode }) => {
  ...
  const convert = () => {
    ...
    const convertedValue = (
      value * weightConversionRates[fromUnit][toUnit]
    ).toFixed(2);
            ...
            >
              {Object.keys(weightConversionRates).map((unit) => (
                <option key={unit} value={unit}>
                  {unitLabels.weight[unit].full} (
                  {unitLabels.weight[unit].short})
                </option>
              ))}
            </select>
          </div>
