...
import { areaConversionRates, unitLabels } from "../utils/constants";
import Header from "../components/Header";

const AreaConverter = ({ isDark, toggleDarkMode }) => {
  ...
  const convert = () => {
    ...
    const convertedValue = (
      value * areaConversionRates[fromUnit][toUnit]
    ).toFixed(2);
            ...
            >
              {Object.keys(areaConversionRates).map((unit) => (
                <option key={unit} value={unit}>
                  {unitLabels.area[unit].full} ({unitLabels.area[unit].short})
                </option>
              ))}
            </select>
          </div>
