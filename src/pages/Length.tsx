...
import { lengthConversionRates, unitLabels } from "../utils/constants";
import Header from "../components/Header";

const LengthConverter = ({ isDark, toggleDarkMode }) => {
  ...
  const convert = () => {
    ...
    const convertedValue = (
      value * lengthConversionRates[fromUnit][toUnit]
    ).toFixed(2);
            ...
            >
              {Object.keys(lengthConversionRates).map((unit) => (
                <option key={unit} value={unit}>
                  {unitLabels.length[unit].full} (
                  {unitLabels.length[unit].short})
                </option>
              ))}
            </select>
          </div>
